...
def training_loop(epochs, model, train_loader, valid_loader, optimizer, loss_fn, lr_scheduler):
    ...
    best_loss = float('inf')   # dùng để lưu loss tốt nhất
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"Epoch: {epoch}/{epochs} | Train: {train_loss:.4f} | Val: {val_loss:.4f} | IoU: {val_IoU:.4f} | Dice: {val_dice:.4f}")

        # 🔥 SAVE BEST MODEL
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), "best_model.pth")
            print(f"🔥 Saved new best model at epoch {epoch} with val_loss: {best_loss:.4f}")
